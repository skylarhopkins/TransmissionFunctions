#'Negative log likelihood when fitting the density-dependent transmission function
#'
#''nll.DD.fn' returns the negative log likelihood (nll) from fitting a density-dependent transmission function
...
#'
#'@param pars named numeric vector. log(B) and log(gamma) in a vector, must be named - usually passed from optim()
...
#'#nll.DD.fn(pars=startpar, initial.inf = initial.I,initial.sus = initial.S,n = samp.sizes,datasets = data,popsizes = pops,time.outs = time.out,time.samps = time.samp)
